...
local function GetFullPathOfAnInstance(instance)
    ...
    if not instance.Parent and instance ~= game then
        return head .. " --[[ 親がnilまたは破棄されました ]] --[[ PARENTED TO NIL OR DESTROYED ]]"
    end
...
BrowserHeaderText.Text = "リモートブラウザ / Remote Browser"
...
HeaderTextLabel.Text = "タートルスパイ / Turtle Spy"
...
Code.Text = "Turtle Spyをご利用いただきありがとうございます！ :D / Thanks for using Turtle Spy! :D"
...
CodeComment.Text = "-- TurtleSpyによって生成されたスクリプト、作成者: Intrer#0421 / -- Script generated by TurtleSpy, made by Intrer#0421"
...
InfoHeaderText.Text = "情報: RemoteFunction / Info: RemoteFunction"
...
CopyCode.Text = "コードをコピー / Copy code"
...
CopyScriptPath.Text = "スクリプトパスをコピー / Copy script path"
...
CopyDecompiled.Text = "逆コンパイルスクリプトをコピー / Copy decompiled script"
...
IgnoreRemote.Text = "リモートを無視 / Ignore remote"
...
BlockRemote.Text = "リモートの発火をブロック / Block remote from firing"
...
WhileLoop.Text = "無限ループスクリプトを生成 / Generate while loop script"
...
CopyReturn.Text = "実行して戻り値をコピー / Execute and copy return value"
...
DoNotStack.Text = "新しい引数で発火されたら積み上げない / Unstack remote when fired with new args"
...
CopyDecompiled.MouseButton1Click:Connect(function()
    ...
    if not isSynapse() then
        CopyDecompiled.Text = "このエクスプロイトは逆コンパイルをサポートしていません！ / This exploit doesn't support decompilation!"
    ...
    if not decompiling and remote and lookingAt then
        ...
        spawn(function()
            while true do
                ...
                CopyDecompiled.Text = "逆コンパイル中. / Decompiling."
                wait(0.8)
                if decompiling == false then return end
                CopyDecompiled.Text = "逆コンパイル中.. / Decompiling.."
                wait(0.8)
        ...
        decompiling = false
        if success[1] then
            CopyDecompiled.Text = "逆コンパイルをコピーしました！ / Copied decompilation!"
            CopyDecompiled.TextColor3 = Color3.fromRGB(76, 209, 55)
        else
            warn(success[2], success[3])
            CopyDecompiled.Text = "逆コンパイルエラー！ F9でエラーを確認してください。 / Decompilation error! Check F9 to see the error."
            CopyDecompiled.TextColor3 = Color3.fromRGB(232, 65, 24)
        end
        wait(1.6)
        CopyDecompiled.Text = "逆コンパイルスクリプトをコピー / Copy decompiled script"
...
BlockRemote.MouseButton1Click:Connect(function()
    ...
    if lookingAt and not bRemote then
        ...
        BlockRemote.Text = "リモートのブロックを解除 / Unblock remote"
...
IgnoreRemote.MouseButton1Click:Connect(function()
    ...
    if lookingAt and not iRemote then
        ...
        IgnoreRemote.Text = "リモートの無視を停止 / Stop ignoring remote"
...
Clear.MouseButton1Click:Connect(function()
    ...
    ButtonEffect(Clear, "クリアしました！ / Cleared!")
end)
...
DoNotStack.MouseButton1Click:Connect(function()
    if lookingAt then
        ...
        if isUnstacked then
            ...
            DoNotStack.Text = "リモートを積み上げる / Stack remote"
...
RemoteScrollFrame.ChildAdded:Connect(function(child)
    ...
    local connection = child.MouseButton1Click:Connect(function()
        ...
        InfoHeaderText.Text = "情報: "..remote.Name.." / Info: "..remote.Name
        if event then 
            InfoButtonsScroll.CanvasSize = UDim2.new(0, 0, 1, 0)
        else
